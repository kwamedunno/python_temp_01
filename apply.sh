...
original_author="kwamedunno"
original_name="python_temp_01"
original_urlname="python_temp_01"
original_description="Awesome python_temp_01 created by kwamedunno" 
...
rm -rf "${name}"
rm -rf tests
cp -R ${TEMPLATE_DIR}/python_temp_01 "${name}"
...
then
    ...
    echo "then run 'python_temp_01' or 'python -m python_temp_01'"
else
    poetry install
    echo "Applied Flask template"
    echo "Ensure you activate your env with 'poetry shell'"
    echo "then run 'python_temp_01' or 'python -m python_temp_01' or 'poetry run python_temp_01'"
fi
